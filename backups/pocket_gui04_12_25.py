...
def fetch_uniprot_metadata(pid: str):
    ...
    try:
        ...
        # protein name
        prot_name = j.get("proteinDescription", {}) \
                     .get("recommendedName", {}) \
                     .get("fullName", {}) \
                     .get("value", "Unknown protein")

        # organism
        organism = j.get("organism", {}) \
...
# Default analyte on first load
if "analyte_query" not in st.session_state:
    st.session_state["analyte_query"] = "glucose"

# ------------------------------------------------------------------
# ANALYTE PROFILES (must be defined before use)
# ------------------------------------------------------------------
ANALYTE_PROFILES = {
    # Metabolites
...
# User can type OR select from the dropdown
selected_analyte = st.selectbox(
    "Analyte name (type to search):",
    options=analyte_options,
    index=analyte_options.index(st.session_state["analyte_query"])
    if st.session_state["analyte_query"] in analyte_options
    else 0,
)

# Save selection into session state
st.session_state["analyte_query"] = selected_analyte
...
if not np.isnan(mw_num):
    # We have a meaningful ligand MW → compute QuickShape
    df_filtered["quickshape_score"] = df_filtered.apply(
        lambda row: compute_quickshape(row, mw=mw_num),
        axis=1,
    )
else:
    # No MW (e.g. only heuristic profile) → leave as NaN
...
with col3:
    ...
    if not df_filtered.empty:
        if (
            ...
            # Use ligand compatibility if available
            value = df_filtered["ligand_compat_score"].dropna().mean() * 100
            mean_value = f"{value:.2f}".rstrip("0").rstrip(".") + "%"
        else:
            # Fallback: biosensor heuristic if we don't have ligand info
...
with viewer_col:
    ...
    with vcol:
        ...
        if analyte_folder_name:
            ...
            # IMPORTANT: match Vina output naming, e.g. P40925_pocket2_glucose_out.pdbqt
            docked_pose = (
                docked_dir
                / f"{prot_id}_pocket{pocket_num}_{analyte_folder_name}_out.pdbqt"
            )

        # Checkbox to toggle ligand visibility if a pose exists
        ...
        if not protein_pdb.exists():
            ...
            try:
                ...
                # Docked ligand (bright orange/yellow) – only if checkbox is on
                if (
                    show_ligand_pose
                    ...
                    # If no ligand or hidden, focus on pocket if available, else whole protein
                    if pocket_pdb.exists():
                        view.zoomTo({"model": 1})
                    else:
                        view.zoomTo()
