...
def compute_quickshape(row: pd.Series, mw: float) -> float:
    ...
    # Gaussian-ish preferences around good ratios
...
def load_pockets() -> pd.DataFrame:
    ...
    def resolve_uniprot_field(pid: str):
        # 1) Try local static table
        if pid in UNIPROT_INFO:
            return UNIPROT_INFO[pid]

        # 2) Otherwise fetch dynamically from UniProt API
...
def fetch_pubchem_sdf_by_name(name: str) -> str | None:
    ...
    try:
        ...
        # CID -> 3D SDF (fall back to 2D if 3D not available)
...
st.write(
    ...
    "biosensors for cell-based systems."
)


# ---------------------------------------------------------------------
# ANALYTE SELECTION (ONLY THING SHOWN INITIALLY)
# ---------------------------------------------------------------------
...
analyte_options = list(ANALYTE_PROFILES.keys())
placeholder = "Type or select an analyte…"

selected_analyte = st.selectbox(
    "Analyte name (type to search):",
    options=[placeholder] + analyte_options,
    index=0,
)

if selected_analyte == placeholder:
    aq = ""
else:
    aq = selected_analyte.lower().strip()

st.session_state["analyte_query"] = aq

if not aq:
    st.info(
        "Start by typing or choosing an analyte above to explore pockets, docking and 3D views."
    )
    st.stop()

# ------------------------------------------------------------------
# ANALYTE PROFILE + LIGAND INFO (PubChem)
# ------------------------------------------------------------------

profile = ANALYTE_PROFILES.get(aq)
ligand_info = fetch_ligand_from_pubchem(aq) if aq else None
...
# ------------------------------------------------------------------
# COLOR THEMES (category pills)
# ------------------------------------------------------------------
CATEGORY_COLORS = {
    "met": ("#E8F0FF", "#1E40AF"),   # metabolites
    "nt":  ("#FDECF3", "#9D174D"),   # neurotransmitters
    "st":  ("#FEF7D1", "#B45309"),   # steroids/hormones
    "dr":  ("#F3F4F6", "#374151"),   # drugs
    "ion": ("#EEE6FF", "#5B21B6"),   # ions
}
...
with col3:
    ...
    st.caption(label)
    st.markdown(
        f"<h2 style='margin: 0;'>{mean_value}</h2>",
        unsafe_allow_html=True,
    )
...
with viewer_col:
    ...
    with vcol:
        ...
        pocket_pdb = (
            STRUCT_DIR / f"{prot_id}_out" / "pockets" / f"pocket{pocket_num}_atm.pdb"
        )

        interpreted_name = st.session_state.get("interpreted_name", None)
        analyte_for_docking_local = (interpreted_name or analyte_for_docking or "").strip()
        analyte_folder_name_local = (
            analyte_for_docking_local.replace(" ", "_")
            if analyte_for_docking_local
            else None
        )

        docked_pose = None
        if analyte_folder_name_local:
            docked_dir = BASE_DIR / "docking" / analyte_folder_name_local / "docked"
            docked_pose = (
                docked_dir
                / f"{prot_id}_pocket{pocket_num}_{analyte_folder_name_local}_out.pdbqt"
            )
...
# ---------------------------------------------------------------------
# DOCKING ANALYSIS + LIGAND 3D + PACKAGE DL
# ---------------------------------------------------------------------

st.markdown("---")
st.subheader("Docking & ligand profile")

mw_val = logp_val = tpsa_val = charge_val = "n/a"
mw_hint = logp_hint = tpsa_hint = charge_hint = None

if ligand_info:
    mw_val = "n/a" if np.isnan(mw_num) else f"{mw_num:.1f} Da"
    logp_val = "n/a" if np.isnan(xlogp_num) else f"{xlogp_num:.2f}"
    tpsa_val = "n/a" if np.isnan(tpsa_num) else f"{tpsa_num:.1f} Å²"
    charge_val = str(int(charge_num)) if not np.isnan(charge_num) else "0"

    mw_hint = describe_mw(mw_num)
    logp_hint = describe_logp(xlogp_num, fallback_polarity=ligand_info.get("polarity"))
...
# Row: Ligand 3D (left) | Docking package button (right)
ligand_col, package_col = st.columns([1.3, 1])
...
# ---------------------------------------------------------------------
# MATCHING POCKETS TABLE (placeholder)
# ---------------------------------------------------------------------
