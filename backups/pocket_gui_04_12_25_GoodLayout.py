...
def build_docking_package_zip(analyte: str) -> tuple[bytes | None, str]:
    ...
    if not docking_subdir.exists() or not targets_csv.exists():
        return None, (
            f"Docking directory or docking_targets.csv missing in {docking_subdir}"
        )
...
# ---------------------------------------------------------------------
# ANALYTE SELECTION + COMMON ANALYTES
# ---------------------------------------------------------------------
...
# Analyte profiles (single definition)
ANALYTE_PROFILES = {
    # Metabolites
...
# ---------------------------------------------------------------------
# LIGAND CHEMICAL PROFILE (compute only here; render later in Docking)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# POCKET FILTERS (collapsed by default)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# MAIN ROW: BEST MATCHING POCKET + 3D VIEWER
# ---------------------------------------------------------------------
...
with best_col:
    ...
    bios_str = (
        "–"
        if np.isnan(bios)
        else f"{(bios * 100):.2f}".rstrip("0").rstrip(".") + "%"
    )
...
with viewer_col:
    ...
    with vcol:
        ...
        if analyte_folder_name:
            docked_dir = DOCKING_ROOT / analyte_folder_name / "docked"
            docked_pose = (
                docked_dir
...
# ---------------------------------------------------------------------
# DOCKING SECTION (properties + package + ligand 3D)
# ---------------------------------------------------------------------

st.markdown("---")
st.subheader("Docking")

# --- Ligand property cards row ---
